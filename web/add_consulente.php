...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Verificar se já existe um consulente com este telefone
        $stmt = $pdo->prepare("SELECT id FROM consulentes WHERE phone = ?");
        $stmt->execute([$phone]);
        
        if ($stmt->fetch()) {
            throw new Exception('Já existe um consulente com este telefone');
        }

        // Verificar se já existe um consulente com este email (se fornecido)
        if ($email) {
            $stmt = $pdo->prepare("SELECT id FROM consulentes WHERE email = ?");
            $stmt->execute([$email]);
            
            if ($stmt->fetch()) {
                throw new Exception('Já existe um consulente com este email');
            }
        }

        // Inserir novo consulente
        $stmt = $pdo->prepare("
        ...
        header('Location: list_consulentes.php?message=Consulente adicionado com sucesso!&type=success');
        exit;
...
<head>
    ...
    <title>ElosTupi - Adicionar Consulente</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <textarea id="notes" name="notes" 
                              placeholder="Informações adicionais sobre o consulente..."><?php echo htmlspecialchars($_POST['notes'] ?? ''); ?></textarea>
                    <div class="help-text">Observações ou informações relevantes</div>
                </div>

                <button type="submit" class="btn">
                    Adicionar Consulente
                </button>
            </form>
