...
class ProductService {
  ...
  Future<Map<String, dynamic>> decrementStock(
    ...
    return await _databaseService.decrementStock(
      productId,
      quantity,
  ...
  Future<Map<String, dynamic>> bulkDecrementStock(
    ...
    return await _databaseService.bulkDecrementStock(
      items,
      isInternal: isInternal,
    );
  }
  ...
  Future<List<String>> getCategoryOrder() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Erro ao carregar ordem das categorias: ${response.statusCode}',
        );
      }
    } catch (e) {
  ...
  Future<List<String>> syncCategories() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Erro ao sincronizar categorias: ${response.statusCode}',
        );
      }
    } catch (e) {
      throw Exception('Erro de conexão: $e');
  ...
  Future<void> updateCategoryOrder(List<String> categories) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception(
          'Erro ao atualizar ordem das categorias: ${response.statusCode}',
        );
      }
    } catch (e) {
      throw Exception('Erro de conexão: $e');
